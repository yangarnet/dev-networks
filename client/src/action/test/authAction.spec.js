...
describe('Async Auth action unit test', () => {
    ...
    describe('[authentication Actions - User Register]', () => {
        it('should dispatch USER_REGISTER_RESOLVE after success', async () => {
            ...
            expect(actionTypes).to.deep.equal(expectedActions.map(action => action.type));

            expect(dispatchedActions.filter(action => action.type === AUTH_ACTION.USER_REGISTER_RESOLVE).payload).to.deep.equal(
                expectedActions.filter(action => action.type === AUTH_ACTION.USER_REGISTER_RESOLVE).payload
            );
        });
        ...
        it('should dispatch USER_REGISTER_REJECT', async () => {
            ...
            expect(actions.filter(action => action.type === AUTH_ACTION.USER_REGISTER_REJECT).payload).to.deep.equal(
                expectedActions.filter(action => action.type === AUTH_ACTION.USER_REGISTER_REJECT).payload
            );
        });
    });
    ...
    describe('[user login]', () => {
        it('should dispatch USER_LOGIN_RESOLVE', async () => {
            ...
            expect(actions.map(action => action.type)).to.deep.equal(expectedActions.map(action => action.type));
            expect(actions.find(action => action.type === AUTH_ACTION.SET_CURRENT_USER)).to.be.not.null;

            expect(actions.find(action => action.type === AUTH_ACTION.SET_CURRENT_USER).payload).to.be.not.null;
        });
        ...
        it('should dispatch USER_LOGIN_REJECT when user not found', async () => {
            ...
            expect(actions.map(action => action.type)).to.deep.equal(expectedActions.map(action => action.type));
            expect(actions.find(action => action.type === AUTH_ACTION.USER_LOGIN_REJECT)).to.be.not.undefined;

            expect(actions.find(action => action.type === AUTH_ACTION.USER_LOGIN_REJECT).payload).to.be.deep.equal(
                expectedActions.find(action => action.type === AUTH_ACTION.USER_LOGIN_REJECT).payload
            );
        });
